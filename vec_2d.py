...
class Vec2d(object):
    ...
    def in_rect(self, corner_1_or_rect, corner_2=None, allow_equal=True):
        """
            check if own position is inside the given rectangle
        """
        if corner_2 is None:
            # handle a [[x1, [y1], [x2, y2]] rectangle in corner_1_or_rect
            corner_1 = corner_1_or_rect[0]
            corner_2 = corner_1_or_rect[1]
        else:
            corner_1 = corner_1_or_rect

        if allow_equal:
            if corner_1.x <= self.x <= corner_2.x:
                if corner_1.y <= self.y <= corner_2.y:
                    return True
        else:
            if corner_1.x < self.x < corner_2.x:
                if corner_1.y < self.y < corner_2.y:
